...
public class TransportConf {
  ...
  public TransportConf(String module, ConfigProvider conf) {
    ...
    //S park.Shuffle.io.preferDirectBufs参数仅Netty使用：堆外缓存可以有效减少垃圾回收和缓存复制。对于堆外内存紧张的用户来说，
    // 可以考虑禁用这个选项，以迫使Netty所有内存都分配在堆上，默认是true。
    SPARK_NETWORK_IO_PREFERDIRECTBUFS_KEY = getConfKey("io.preferDirectBufs");
    SPARK_NETWORK_IO_CONNECTIONTIMEOUT_KEY = getConfKey("io.connectionTimeout");
    SPARK_NETWORK_IO_BACKLOG_KEY = getConfKey("io.backLog");
    // Spark.Shuffle.io.numConnectionsPerPeer（仅Netty使用）
    // 重新使用主机之间的连接，以减少大型集群的连接建立。对于具有多个硬盘和少量主机的集群，
    // 这可能导致并发性不足，以使所有磁盘饱和，因此用户可考虑增加此值，默认是1次。
    ...
    // 默认重试次数为3，调整获取Shuffle数据的重试次数，通常建议增加重试次数到8~10次
    SPARK_NETWORK_IO_MAXRETRIES_KEY = getConfKey("io.maxRetries");
    // Spark.Shuffle.io.retryWait默认值：5s。
    // 参数说明：代表了每次重试拉取数据的等待间隔，默认是5s，在Spark-conf配置文件中配置。
    // 调优建议：默认情况下，重试3次，每次重试的间隔时间为5s，重试引起的最大延迟为15s，以maxRetries×retryWait计算。
    //  建议加大间隔时长（如60s），以增加Shuffle操作的稳定性。
